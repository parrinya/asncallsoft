<%@ Page Title="" Language="VB" MasterPageFile="~/MasterPage/Page/Index/MasterPage.master" AutoEventWireup="false" CodeFile="frmEditCredit.aspx.vb" Inherits="Modules_Manager_Manage_Case_frmEditCredit" %>
...
<asp:UpdatePanel ID="UpdatePanel3" runat="server" >   
    ...
    <iframe src="<%= strLinkCredit %>" frameborder="0" height="800" width="100%"></iframe>
    </div>   
    </ContentTemplate>
</asp:UpdatePanel> 
<asp:SqlDataSource ID="SqlLead" runat="server" 
    ...
    <asp:SqlDataSource ID="SqlSup" runat="server" 
     ...
     <asp:SqlDataSource ID="SqlNameCarID" runat="server" 
        ...
        ,isnull((select sum([TotalPay]) from [TblAppPay] where  Ispaid=1 and [AppID] =TblApplication.AppID),0) as Pay
        ...
        WHERE (TblApplication.Statusqc  not IN (8,9,10)) 
        AND (TblApplication.AppStatus = 1) And 
        TblUser.SupID=@SupID  AND  
        TblUser.LeaderID=@LeaderID And   
        TblApplication.CreateID=@TsrID
        and isnull((select sum([TotalPay]) from [TblAppPay] where  Ispaid=1 and [AppID] =TblApplication.AppID),0)=0 
