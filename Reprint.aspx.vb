...
Partial Class Reprint
    ...
    Private Sub ShowData()
        ...
        If (DropDownList1.SelectedIndex >= 0 And TextBox1.Text.Trim = "") Then
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "Alert", "alert('กรุณาระบุคำค้นหา เช่น ทะเบียน ชื่อ-สกุล หรือ AppID !!');", True)
            Exit Sub
        End If

        If DropDownList1.SelectedIndex = 0 Then tmp2 = " TblCar.CarID Like '%" & TextBox1.Text.Trim & "%'"

        If DropDownList1.SelectedIndex = 1 Then tmp2 = " (LTRIM(RTRIM(TblCustomer.fnameth)) + LTRIM(RTRIM(TblCustomer.lnameth)) like '%" & Trim(TextBox1.Text.Replace(" ", "")) & "%')"

        If DropDownList1.SelectedIndex = 2 Then tmp2 = " TblApplication.AppID = '" & TextBox1.Text.Trim & "'"

        Dim str As String = " SELECT TblApplication.AppID AS [AppID], TblCar.CarID AS [ทะเบียนรถ], TblCustomer.FNameTH + '  ' + TblCustomer.LNameTH AS [ชื่อ-นามสกุล ลูกค้า], TblApplication.SubmitDate AS [วันที่ Submit] ,TblApplication.PolicyNo + ' || ' + Tbl_ProductType.DESC1   AS [เลขกรมธรรม์ || บ.ประกัน],  CASE TblApplication.AppStatus WHEN 0 THEN '<FONT Color=""red"">ไม่ใช้งาน</FONT>' WHEN 1 THEN '<FONT Color=""DarkGreen"">ใช้งาน</FONT>' ELSE 'ไม่ระบุ' END AS [สถานะ] ,TblCar.IDCar" + _
                            " FROM TblApplication INNER JOIN TblCar ON TblApplication.IdCar = TblCar.IdCar INNER JOIN TblCustomer ON TblCustomer.CusID = TblCar.CusID INNER JOIN Tbl_ProductType ON Tbl_ProductType.ProTypeID = TblApplication.ProDuctID" + _
                            " WHERE  " + tmp2 + " ORDER BY TblApplication.AppID DESC"

        Conn.Open()
        Command = New SqlCommand(str, Conn)
        DataReader = Command.ExecuteReader()

        UWGShowEmp.DataSource = DataReader
        UWGShowEmp.DataBind()

        DataReader.Close()
        Conn.Close()

        If UWGShowEmp.Rows.Count > 0 Then

            UWGShowEmp.Columns(0).Width = 70
            UWGShowEmp.Columns(0).CellStyle.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(0).Header.Style.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(1).Width = 100
            UWGShowEmp.Columns(1).CellStyle.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(1).Header.Style.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(2).Width = 240
            UWGShowEmp.Columns(2).Header.Style.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(3).Width = 150
            UWGShowEmp.Columns(3).CellStyle.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(3).Header.Style.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(4).Width = 250
            UWGShowEmp.Columns(4).CellStyle.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(4).Header.Style.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(5).Width = 80
            UWGShowEmp.Columns(5).Header.Style.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(5).CellStyle.HorizontalAlign = HorizontalAlign.Center
            UWGShowEmp.Columns(6).Width = 0
            UWGShowEmp.Columns(6).Header.Style.Width = 0


        Else
            UWGShowEmp.DataSource = Nothing
            UWGShowEmp.DataBind()
            UWGShowEmp.DisplayLayout.Strings.NoDataMessage = "<FONT Color='RED' Size='2'>ไม่พบข้อมูลที่ค้นหา กรุณาตรวจสอบเงื่อนไข !!!</FONT>"
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "Alert", "alert('ไม่พข้อมูลที่ค้นหา กรุณาตรวจสอบเงื่อนไข !!!');", True)
        End If
    End Sub
    ...
    Private Sub SetappAcc()
        ...
        str = "insert into tmp_QC_app01 ( " + _
        ...
        " Car_Fire, Acc_Lost1, Acc_Lost2, Acc_Lost3, Acc_Lost4, Maintain, Insure, Apprela, Pledge,ASNcomt, typereport,  cartypename, fname, lname, ProTypeBrand,  a1 , a2, a3, initth,expprotectdate,sname,UserID ) " + _
        " values ( " + _
        ...
        " '" & dt.Rows(0).Item("88") & "','" & dt.Rows(0).Item("89") & "','" & dt.Rows(0).Item("90") & "','" & dt.Rows(0).Item("91") & "','" & dt.Rows(0).Item("92") & "','" & dt.Rows(0).Item("93") & "','" & dt.Rows(0).Item("94") & "','" & dt.Rows(0).Item("95") & "','" & dt.Rows(0).Item("96") & "','" & dt.Rows(0).Item("expprotectdate") & "','" & sName & "','" & Request.Cookies("UserID").Value & "') "
  ...
  " ) a inner join  Tbl_Cartype b  on a.cartype =  b.cartypeid) a inner join tbluser b on a.assignto = b.userid ) a inner join Tbl_ProductType b on a.productid = b.protypeid )  a  inner join TblCustomerInit b  on a.initid = b.initid ) m left join " + _
   " (select appid appid1 ,Typepay Typepay1,convert(int,payid) payid1,AppointDate AppointDate1,totalpay totalpay1 from tblapppay where payid = 1) a on m.appid = a.appid1 left join " + _
      ...
      " (select appid appid6 ,Typepay Typepay6,convert(int,payid) payid6,AppointDate AppointDate6,totalpay totalpay6 from tblapppay where payid = 6) f on m.appid = f.appid6 "
        ...
        str = " insert  into  tmp_QC_PayCredit (" + _
        ...
        " '" & dt2.Rows(0).Item("10") & "','" & dt2.Rows(0).Item("11") & "','" & dt2.Rows(0).Item("12") & "','" & dt2.Rows(0).Item("13") & "','" & dt2.Rows(0).Item("14") & "','" & dt2.Rows(0).Item("15") & "','" & dt2.Rows(0).Item("16") & "','" & dt2.Rows(0).Item("17") & "','" & FunAll.chkNull(dt2.Rows(0).Item("18"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("19"), 0) & "', " + _
        " '" & FunAll.chkNull(dt2.Rows(0).Item("20"), 0) & "','" & d1 & "','" & FunAll.chkNull(dt2.Rows(0).Item("22"), 1) & "','" & FunAll.chkNull(dt2.Rows(0).Item("23"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("24"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("25"), 0) & "','" & d2 & "','" & FunAll.chkNull(dt2.Rows(0).Item("27"), 1) & "','" & FunAll.chkNull(dt2.Rows(0).Item("28"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("29"), 0) & "', " + _
        " '" & FunAll.chkNull(dt2.Rows(0).Item("30"), 0) & "','" & d3 & "','" & FunAll.chkNull(dt2.Rows(0).Item("32"), 1) & "','" & FunAll.chkNull(dt2.Rows(0).Item("33"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("34"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("35"), 0) & "','" & d4 & "','" & FunAll.chkNull(dt2.Rows(0).Item("37"), 1) & "' ,'" & FunAll.chkNull(dt2.Rows(0).Item("38"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("39"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("40"), 0) & "','" & d5 & "','" & FunAll.chkNull(dt2.Rows(0).Item("42"), 1) & "' ,'" & FunAll.chkNull(dt2.Rows(0).Item("43"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("44"), 0) & "','" & FunAll.chkNull(dt2.Rows(0).Item("45"), 0) & "','" & d6 & "','" & FunAll.chkNull(dt2.Rows(0).Item("47"), 1) & "','" & Request.Cookies("UserID").Value & "' ) "
        Command = New SqlCommand(str, Conn)
        Command.ExecuteNonQuery()
    ...
    Private Sub Print_CV_Payment()
        ...
        'CrystalReportViewer1.SelectionFormula = "{tmppayment.userid} = '" & Request.Cookies("userID").Value & "'"
        'CrystalReportViewer1.RefreshReport()

        myReport.PrintOptions.PrinterName = "cvRPT" '"cvRPT"
        'myReport.PrintOptions.PrinterName = "RICOH MPC5000 PCL"
        myReport.PrintToPrinter(1, False, 1, 1)

        'If dt.Rows.Count > 0 Then
        'If dt.Rows(0).Item("senddoc") = 1 Then
        '    Dim reportname2 As String = Server.MapPath("rptdoc1.rpt")
        '    myReport.Load(reportname2)
        '    myReport.SetDatabaseLogon(users, pass)
        '    myReport.SetParameterValue("User", Request.Cookies("UserID").Value)
        '    myReport.PrintOptions.PrinterName = "cvRPT" '"cvRPT"
        ...
        ScriptManager.RegisterStartupScript(Page, Page.GetType, "Message", "alert('ออกใบคำขอเอาประกันภัยของ AppID " & lblApp.Text.Trim & " เรียบร้อยแล้ว !!');", True)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            WebTab1.VisibleHeader = False
        End If

        'TextBox1.Attributes.Add("onkeyDown", "return AnyInput_KeyDown(event,'btnDisplay')")
    End Sub

    Protected Sub btnDisplay_Click(ByVal sender As Object, ByVal e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnDisplay.Click
        ShowData()
    End Sub

    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnPrint.Click
        If lblApp.Text.Trim = "" Then
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "Alert", "alert('กรุณาเลือกข้อมูลเพื่อออกใบคำขอ หรือใช้งาน App Store !!');", True)
            Exit Sub
        End If
        SetappAcc()
        Print_CV_Payment()
    ...
    Protected Sub UWGShowEmp_SelectedRowsChange(ByVal sender As Object, ByVal e As Infragistics.WebUI.UltraWebGrid.SelectedRowsEventArgs) Handles UWGShowEmp.SelectedRowsChange
        If e.SelectedRows.Count = 1 Then
            lblApp.Text = e.SelectedRows.Item(0).Cells(0).Text
            lblOldApp.Text = FunAll.GetOldAppID(e.SelectedRows.Item(0).Cells(6).Text)

            lblInfo.Text = "ข้อมูล : <FONT Color='Blue'>AppID " & e.SelectedRows.Item(0).Cells(0).Text & "</FONT> || <FONT Color='Purple'>ชื่อ-สกุลลูกค้า " & e.SelectedRows.Item(0).Cells(2).Text & "</FONT> || <FONT Color='DarkGreen'>ทะเบียนรถ " & e.SelectedRows.Item(0).Cells(1).Text & "</FONT> >> <FONT Color='RED'>ออกใบคำขอเอาประกันภัย</FONT>"
            If lblApp.Text <> "" Then
                btnPrint.Enabled = True
                btnOpenFile.Enabled = True
                btnGenCV_PY.Enabled = True
            End If
        End If
    ...
    Protected Sub btnOpenFile_Click(ByVal sender As Object, ByVal e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnOpenFile.Click
        If lblOldApp.Text = "" Then
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "AppStore", "OpenAppStore(" & lblApp.Text & ",0," & Request.Cookies("UserID").Value & ")", True)
        Else
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "AppStore", "OpenAppStore(" & lblApp.Text & "," & lblOldApp.Text & "," & Request.Cookies("UserID").Value & ")", True)
        End If
    End Sub
    Private Sub Auto_File()
        ...
        If dttmpLine.Rows.Count = 1 Then
            ...
            'save barcode
            Dim destPathbarcode As String = "D:\\LINE\\" + lblApp.Text + "\barcode.bmp"
            Dim bmbarcode As New Bitmap(myimg)
            bmbarcode.Save(destPathbarcode)
            bmbarcode.Dispose()

            'save barCode
            Dim destPathqrCode As String = "D:\\LINE\\" + lblApp.Text + "\qrcode.bmp"
            Dim bmqrCode As New Bitmap(myimg1)
            bmqrCode.Save(destPathqrCode)
            bmqrCode.Dispose()
    ...
    Protected Sub btnGenCV_PY_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnGenCV_PY.Click
        If lblApp.Text.Trim = "" Then
            ScriptManager.RegisterStartupScript(Page, Page.GetType, "Alert", "alert('กรุณาเลือกข้อมูลเพื่อ Gen File ภาพ !!');", True)
            Exit Sub
        End If
        SetappAcc()
        Auto_File()
